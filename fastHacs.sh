...
echo "Перезапустить контейнер ${CONTAINER_NAME}? [Y/n] (авто-перезапуск через 15 секунд)"

# Запускаем обратный отсчёт
for ((i=15; i>0; i--)); do
    read -t 1 -n 1 -p "⏳ Перезапуск через ${i} сек... Нажмите Y для отмены: " REPLY
    if [[ "$REPLY" =~ ^[Nn]$ ]]; then
        echo -e "\n❌ Перезапуск отменён."
...
echo -e "\n🔄 Перезапускаем контейнер ${CONTAINER_NAME}..."
